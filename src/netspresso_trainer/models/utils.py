...
DEFAULT_CACHE_DIR = (
    Path.home() / ".cache" / f"{__name__.split('.')[0]}"
)  # ~/.cache/netspresso_trainer
...
def download_model_checkpoint(
    ...
    checkpoint_filename = Path(checkpoint_url).name.split("?versionId")[
        0
    ]  # @illian01: Remove specified version id
...
def load_from_checkpoint(
    ...
    if model_checkpoint is None:
        ...
        ), "When `use_pretrain` is True, model_name should be given."
        assert (
            model_name in MODEL_CHECKPOINT_URL_DICT
        ...
        logger.info(
            f"Pretrained model for {model_name} is loaded from: {model_checkpoint}"
        )
    ...
    if not load_checkpoint_head:
        logger.info("-" * 40)
        logger.info(
            "Head weights are not loaded because model.checkpoint.load_head is set to False"
        )
    ...
    if not load_checkpoint_head:
        missing_keys = [
            key for key in missing_keys if not key.startswith(model.head_list)
        ]
...
def set_training_targets(train_model, targets: dict):
    if (
        isinstance(train_model, nn.Module)
        and hasattr(train_model, "head")
        and hasattr(train_model.head, "_training_targets")
    ):
        train_model.head.set_training_targets(targets)
    elif (
        isinstance(train_model, torch.nn.parallel.DistributedDataParallel)
        and hasattr(train_model.module, "head")
        and hasattr(train_model.module.head, "_training_targets")
    ):
