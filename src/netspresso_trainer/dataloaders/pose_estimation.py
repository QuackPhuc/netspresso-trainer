...
class PoseEstimationSampleLoader(BaseSampleLoader):
    def __init__(self, conf_data, train_valid_split_ratio):
        super(PoseEstimationSampleLoader, self).__init__(
            conf_data, train_valid_split_ratio
        )
    ...
    def load_data(self, split="train"):
        assert split in [
            ...
            "valid",
            "test",
        ], f"split should be either {['train', 'valid', 'test']}."
        ...
        annotation_dir: Optional[Path] = (
            data_root / split_dir.label if split_dir.label is not None else None
        )
        images: List[str] = []
        labels: List[str] = []
        ...
        if annotation_dir is not None:
            for ext in IMG_EXTENSIONS:
                for file in chain(
                    image_dir.glob(f"*{ext}"), image_dir.glob(f"*{ext.upper()}")
                ):
            ...
            for ext in IMG_EXTENSIONS:
                images_and_targets.extend(
                    [
                        {"image": str(file), "label": None, "name": file.stem}
                        for file in chain(
                            image_dir.glob(f"*{ext}"), image_dir.glob(f"*{ext.upper()}")
                        )
                    ]
            ...
            images_and_targets = sorted(
                images_and_targets, key=lambda k: natural_key(k["image"])
            )
    ...
    def load_id_mapping(self):
        ...
        if isinstance(self.conf_data.id_mapping, ListConfig):
            ...
            raise ValueError(
                f"Invalid id_mapping type: {type(self.conf_data.id_mapping)}"
            )
...
class PoseEstimationCustomDataset(BaseCustomDataset):
    ...
    def cache_dataset(self):
        ...
        load_imgs = ThreadPool(num_threads).imap(
            partial(_load, samples=self.samples), all_indices
        )
    ...
    def __getitem__(self, index):
        ...
        ann = self.samples[index][
            "label"
        ]  # TODO: Pose estimation is not assuming that label can be None now
        ...
        outputs.update({"name": self.samples[index]["name"]})
        if ann is None:
